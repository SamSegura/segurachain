...
namespace SeguraChain_Desktop_Wallet.MainForm.System
{
    public class ClassWalletRecentTransactionHistorySystemInstance
    {
                    ...
                    if (walletDataObject != null)
                    {
                        if (!walletDataObject.WalletEnableRescan)
                        {
                            if (walletDataObject.WalletLastBlockHeightSynced >= ClassDesktopWalletCommonData.WalletSyncSystem.GetLastBlockHeightUnlockedSynced(cancellation))
                            {
                                ...
                                if (countMemPoolTransactionIndexed != _lastWalletMemPoolTransactionCount)
                                {
                                    requireUpdate = true;
                                }

                                if (countTransactionIndexed != _lastWalletTransactionCount)
                                {
                                    requireUpdate = true;
                                }
                                ...
                                if (!requireUpdate)
                                {
                                    ...
                                    {
                                        if (countTransactionIndexed > 0 | countMemPoolTransactionIndexed > 0)
                                        {
                                            requireUpdate = true;
                                        }
                                    }
                                ...
                                if (requireUpdate)
                                {
                                    ...
                                    {
                                        await _semaphoreRecentTransactionHistoryAccess.WaitAsync();
                                    }
                                    useSemaphore = true;
        ...
        private void DrawTransactionToRecentHistory(ClassBlockTransaction blockTransaction, string walletAddress, bool isMemPool, int totalTxDrawed)
        {
            ...
            {
                recentTransactionHistoryObject.IsSender = true;
            }
