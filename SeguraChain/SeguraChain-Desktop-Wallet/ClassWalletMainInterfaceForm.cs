...
namespace SeguraChain_Desktop_Wallet
{
    public partial class ClassWalletMainInterfaceForm : Form
    {
        ...
        private void ClassWalletMainInterfaceForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            _walletTransactionHistorySystemInstance.ClearTransactionHistory();
        ...
        private void StartLoadFirstWallet()
        {
            ...
            {
                if (ClassDesktopWalletCommonData.WalletDatabase.LoadWalletFileAsync(ClassWalletDefaultSetting.WalletDefaultFilename).Result == ClassWalletLoadFileEnumResult.WALLET_LOAD_SUCCESS)
                {
#if DEBUG
            ...
            else
            {
                ...
                {
                    if (ClassDesktopWalletCommonData.WalletDatabase.LoadWalletFileAsync(walletFilename).Result == ClassWalletLoadFileEnumResult.WALLET_LOAD_SUCCESS)
                    {
#if DEBUG
                        ...
                        try
                        {
                            foreach (var walletFile in Directory.GetFiles(ClassDesktopWalletCommonData.WalletSettingObject.WalletDirectoryPath, ClassWalletDefaultSetting.WalletFileFormat))
                            {
                                ...
                                if (!containWalletFile)
                                {
                                    ...
                                    foreach (var walletItem in openWalletToolStripMenuItem.DropDownItems)
                                    {
        ...
        private void EnableTaskUpdateBlockchainNetworkStats()
        {
            try
            {
                Task.Factory.StartNew(async () =>
                {
                    while (ClassDesktopWalletCommonData.DesktopWalletStarted)
                    {
                        ClassBlockchainNetworkStatsObject blockchainNetworkStatsObject = ClassDesktopWalletCommonData.WalletSyncSystem.GetBlockchainNetworkStatsObject();
                        ...
                        if (blockchainNetworkStatsObject != null)
                        {
                            ...
                            MethodInvoker invoke = () =>
                            {
                                ...
                                // Synced part.
                                labelMainInterfaceNetworkStatsTotalTransactionMemPoolText.Text = _walletMainFormLanguageObject.LABEL_MAIN_INTERFACE_NETWORK_STATS_TOTAL_TRANSACTION_MEMPOOL_TEXT + ClassDesktopWalletCommonData.WalletSyncSystem.GetTotalMemPoolTransactionFromSync();
                ...
                Task.Factory.StartNew(async () =>
                {
                    ...
                    while (ClassDesktopWalletCommonData.DesktopWalletStarted)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                if (await _walletRecentTransactionHistorySystemInstance.UpdateRecentTransactionHistory(_currentWalletFilename, _cancellationTokenTaskUpdateWalletContentInformations))
                                {
                                    bool complete = false;
                                    MethodInvoker invoke = () =>
                                    {
        ...
        private void panelTransactionHistory_Click(object sender, EventArgs e)
        {
            if (_walletTransactionHistorySystemInstance != null)
            {
                if (_walletTransactionHistorySystemInstance.ContainsTransactionHistoryToWalletFileOpened(_currentWalletFilename))
                {
                    if (!_walletTransactionHistorySystemInstance.GetLoadStatus(_currentWalletFilename, out _))
                    {
                        var position = panelTransactionHistory.PointToClient(Cursor.Position);

                        _walletTransactionHistorySystemInstance.EnableTransactionHoverByClick(_currentWalletFilename, position, _cancellationTokenTaskUpdateWalletContentInformations, out string _, out Rectangle _);
                    }
                }
            }
        }
            ...
            if (_walletTransactionHistorySystemInstance != null)
            {
                if (_walletTransactionHistorySystemInstance.ContainsTransactionHistoryToWalletFileOpened(_currentWalletFilename))
                {
                    if (!_walletTransactionHistorySystemInstance.GetLoadStatus(_currentWalletFilename, out _))
                    {
                        ...
                        _walletTransactionHistorySystemInstance.EnableTransactionHoverByPosition(_currentWalletFilename, position, _cancellationTokenTaskUpdateWalletContentInformations, out _, out _);
                    }
                }
            }
        }
                ...
                if (!_walletTransactionHistorySystemInstance.GetLoadStatus(_currentWalletFilename, out _))
                {
                    ...
                    if (continueExport)
                    {
                        ...
                        })
                        {
                            ...
                            {
                                if (!saveTransactionExportFileDialog.FileName.IsNullOrEmpty())
                                {
        ...
        private void panelInternalRecentTransactions_Click(object sender, EventArgs e)
        {
            if (_walletRecentTransactionHistorySystemInstance != null)
            {
                ...
                _walletRecentTransactionHistorySystemInstance.GetBlockTransactionByClick(panelInternalRecentTransactions.PointToClient(Cursor.Position), walletAddress, _cancellationTokenTaskUpdateWalletContentInformations, out ClassBlockTransaction blockTransaction, out bool isMemPool);
        ...
        private void textBoxSendTransactionAmountSelected_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (_cancellationTokenTaskUpdateWalletContentInformations != null)
            {
                if (!_sendTransactionOnCalculationFeeCost)
                {
                    ...
                    {
                        _semaphoreSendTransactionUpdateFeeCost.Wait();
                        ...
                        try
                        {
                            Task taskUpdateFeeCostEstimated = UpdateTransactionFeeCostEstimated();
                            taskUpdateFeeCostEstimated.Wait(_cancellationTokenTaskUpdateWalletContentInformations.Token);
                        }
                        catch
                        {
                        ...
                        _semaphoreSendTransactionUpdateFeeCost.Release();
                    }
                }
            }
        ...
        private async Task UpdateTransactionFeeCostEstimated()
        {
            ...
            if (!textBoxSendTransactionAmountSelected.Text.IsNullOrEmpty())
            {
            ...
            {
                CleanAmountAndFeeEstimations(false);
                textBoxSendTransactionAmountSelected.ForeColor = Color.Black;
            }
        }
        ...
        private void UpdateTransactionConfirmationTimeEstimated()
        {
            ...
            {
                if (!textBoxSendTransactionConfirmationsCountTarget.Text.IsNullOrEmpty())
                {
        ...
        private bool StartSendTransactionTask(out bool error)
        {
            ...
            if (textBoxSendTransactionWalletAddressTarget.Text.IsNullOrEmpty())
            {
                return false;
            }
            ...
            if (textBoxSendTransactionFeeCalculated.Text.IsNullOrEmpty())
            {
                return false;
            }
            ...
            if (walletPrivateKey.IsNullOrEmpty())
            {
                return false;
            }
        ...
        private void buttonSaveQrCodeReceiveTransactionWalletAddress_Click(object sender, EventArgs e)
        {
            if (panelQrCodeWalletAddress.BackgroundImage != null)
            {
                ...
                })
                {
                    ...
                    {
                        if (!saveQrCodeFileDialog.FileName.IsNullOrEmpty())
                        {
