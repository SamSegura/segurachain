                ...
                if (!dataRead.IsNullOrEmpty(out _))
                {
                    if (ClassUtility.TryDeserialize(dataRead, out soloMinerSettingObject))
                    {
                            ...
                            soloMinerSettingObject?.SoloMinerNetworkSetting != null)
                        {
                            ...
                            {
                                if (CheckMaxThread(soloMinerSettingObject.SoloMinerThreadSetting.max_thread, true))
                                {
                                    if (CheckThreadPriority(soloMinerSettingObject.SoloMinerThreadSetting.thread_priority, true))
                                    {
                                        if (CheckPeerIpTarget(soloMinerSettingObject.SoloMinerNetworkSetting.peer_ip_target, true))
                                        {
                                            if (CheckPeerApiPortTarget(soloMinerSettingObject.SoloMinerNetworkSetting.peer_api_port_target, true))
                                            {
