    ...
    public class BlockchainMemoryManagement
    {
                    ...
                    case CacheBlockMemoryInsertEnumType.INSERT_IN_ACTIVE_MEMORY_OBJECT:
                        {
                            if (ContainsKey(blockHeight))
                            {
                                _dictionaryBlockObjectMemory[blockHeight].Content = value;
                                _dictionaryBlockObjectMemory[blockHeight].CacheUpdated = false;
                                _dictionaryBlockObjectMemory[blockHeight].Content.BlockLastChangeTimestamp = ClassUtility.GetCurrentTimestampInMillisecond();
                                _dictionaryBlockObjectMemory[blockHeight].ObjectCacheType = CacheBlockMemoryEnumState.IN_ACTIVE_MEMORY;
                            }
                            else
                            {
        ...
        public async Task<bool> InsertOrUpdateBlockObjectToCache(ClassBlockObject blockObject, bool keepAlive, CancellationTokenSource cancellation)
        {
            ...
            else
            {
                ...
                if (_dictionaryBlockObjectMemory[blockObject.BlockHeight].Content != null || keepAlive || blockObject.BlockHeight == BlockchainSetting.GenesisBlockHeight)
                {
            ...
            if (_blockchainDatabaseSetting.BlockchainCacheSetting.EnableCacheDatabase && blockObject.BlockHeight > BlockchainSetting.GenesisBlockHeight)
            {
            ...
            try
            {
                if (Count > 0)
                {
                    ...
                    if (lastBlockHeight >= BlockchainSetting.GenesisBlockHeight)
                    {
                        ...
                        if (blockchainNetworkStatsObject.LastBlockHeightTransactionConfirmationDone != lastBlockHeightTransactionConfirmationDone)
                        {
                            bool canceled = false;
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (totalTask > 0)
                        {
                            ...
                            foreach (var blockRange in BuildListBlockHeightByRange(lastBlockHeightUnlockedChecked, listBlockHeightFullyConfirmed, cancellation))
                            {
                                cancellation.Token.ThrowIfCancellationRequested();
                                                    ...
                                                    if (listBlockObjectUpdated.Count > 0)
                                                    {
                                                        ...
                                                        else
                                                        {
                                                            foreach (long blockHeight in listBlockObjectUpdated.GetList.Keys.ToArray())
                                                            {
                                                                ...
                                                                AddOrUpdateBlockMirrorObject(listBlockObjectUpdated[blockHeight]);
                    ...
                    for (long i = lastBlockHeightFromTransaction; i < maxBlockHeightTarget; i++)
                    {
                        ...
                        if (blockHeight > lastBlockHeightWalletCheckpoint && blockHeight <= maxBlockHeightTarget)
                        {
                            ...
                            if (listBlockObjectUpdated != null)
                                listTxFromCacheOrMemory = listBlockObjectUpdated.ContainsKey(blockHeight) ? new DisposableList<ClassBlockTransaction>(false, 0, listBlockObjectUpdated[blockHeight].BlockTransactions.Values.ToArray()) : new DisposableList<ClassBlockTransaction>(false, 0, (await GetTransactionListFromBlockHeightTargetFromMemoryDataCache(blockHeight, false, cancellation))?.Values.ToArray());
                            else
                                listTxFromCacheOrMemory = new DisposableList<ClassBlockTransaction>(false, 0, (await GetTransactionListFromBlockHeightTargetFromMemoryDataCache(blockHeight, false, cancellation))?.Values.ToArray());
        ...
        private async Task<DisposableList<ClassBlockTransaction>> GetListBlockTransactionByListTransactionHashFromMemoryDataCache(List<string> listTransactionHash, long blockHeight, bool useBlockTransactionCache, bool keepAlive, CancellationTokenSource cancellation)
        {
            ...
            if (useBlockTransactionCache)
            {
                listBlockTransaction.GetList = await GetListBlockTransactionCache(listTransactionHash, blockHeight, cancellation);
        ...
        /// <param name="cancellation"></param>
        /// <returns></returns>
        private async Task<SortedList<long, ClassBlockObject>> GetBlockInformationListByBlockHeightListTargetFromMemoryDataCache(DisposableList<long> listBlockHeight, CancellationTokenSource cancellation)
        {
        ...
        private async Task<ClassBlockObject> GetBlockMirrorObject(long blockHeight, CancellationTokenSource cancellation)
        {
            ...
            if (_dictionaryBlockObjectMemory[blockHeight].Content != null)
            {
                _dictionaryBlockObjectMemory[blockHeight].Content.DeepCloneBlockObject(false, out ClassBlockObject blockObjectInformation);
                if (blockObjectInformation != null)
                    return blockObjectInformation;
            }
